 ...
 */
class DelayedForwarder<T> extends Thread {
    ...
    DelayedForwarder(String id, Producer<T> source, Consumer<T> destination) {
        this.id = id;
        this.source = source;
        this.destination = destination;
    }

    DelayedForwarder(int id, Producer<T> source, Consumer<T> destination) {
        ...
        while (true) {
            try {
                ...
                throw new RuntimeException(String.format(
                        "DelayedForwarder %s stopped; it was interrupted!", id), e);
            }
        }
    }
